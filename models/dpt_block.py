...
from croco.models.layers import ReLu
    ...
    def __call__(self, *xs):
        ...
        if len(xs) == 2:
            ...
            if self.width_ratio != 1:
                res = tinygrad.Tensor.interpolate(res, size=(output.shape[2], output.shape[3]), mode='linear')

            output = tinygrad.Tensor.add(output, res)
            # output += res
        ...
        if self.width_ratio != 1:
            ...
            output  = tinygrad.Tensor.interpolate(output, size=(2* output.shape[2], shape), mode='linear')
        else:
            scale_factor=2
            _, _, height, width = output.shape
            size = (int(height * scale_factor), int(width * scale_factor))
            output = tinygrad.Tensor.interpolate(output, size=size,
...
class ResidualConvUnit_custom:
    ...
    def __call__(self, x):
        ...
        return tinygrad.Tensor.add(out, x)
...
class Interpolate:
    ...
    def __init__(self, scale_factor, mode, align_corners=False):
        ...
        self.interp = tinygrad.Tensor.interpolate
...
class DPTOutputAdapter:
    ...
    def __init__(self,
        ...
        self.scratch = Scratch(layer_dims, feature_dim, groups=1, expand=False)
        ...
        if self.head_type == 'regression':
            ...
            self.head = [
                ...
                Interpolate(scale_factor=2, mode="linear", align_corners=True),
    ...
    def adapt_tokens(self, encoder_tokens):
        ...
        x = tinygrad.Tensor.cat(*x, dim=-1)
        return x

    def __call__(self, encoder_tokens: List[tinygrad.Tensor], image_size):
            #input_info: Dict):
        ...
        # Output head
        out = self.head(path_1) # -> this isn't going to work because this is just a list, so we need to use the sequential function, don't make a class
